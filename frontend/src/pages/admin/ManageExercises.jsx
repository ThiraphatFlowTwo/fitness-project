...
import {
  Dumbbell, Plus, Edit2, Trash2, X, CheckCircle,
} from "lucide-react";

const IMAGE_URL = "http://localhost:5000/uploads/";

const EXERCISE_TYPES = [
  "ทุกส่วนของร่างกาย", "ทั่วทั้งร่างกาย", "หลัง", "หน้าอก",
  "หัวไหล่", "ขา", "แขน", "แกนกลางลำตัว", "คาร์ดิโอ",
];
...
const EXERCISE_CATEGORIES = [
  { value: "weight",     label: "ใช้น้ำหนัก (kg)",          desc: "Squat, Bench Press, Deadlift"  },
  { value: "cardio",     label: "คาร์ดิโอ (ระยะทาง/เวลา)",  desc: "วิ่ง, ปั่นจักรยาน, Rowing"     },
  { value: "bodyweight", label: "น้ำหนักตัว (ครั้ง/วินาที)", desc: "Push-up, Pull-up, Plank"       },
  { value: "duration",   label: "เวลาอย่างเดียว (วินาที)",   desc: "Stretching, Yoga, Meditation"  },
];
...
export default function ManageExercises() {
  ...
  const validate = () => {
    const newErrors = {};
    if (!form.exercise_name.trim()) newErrors.exercise_name = "กรุณากรอกชื่อท่า";
  ...
  const filteredExercises = exercises.filter((ex) => {
    ...
    const matchSearch  = ex.exercise_name.toLowerCase().includes(keyword) ||
    ...
    const matchEquip   = equipmentFilter === "all" || ex.equipment_type   === equipmentFilter;
    const matchCat     = categoryFilter  === "all" || ex.exercise_category === categoryFilter;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-800">จัดการท่าออกกำลังกาย</h1>
              <p className="text-slate-600 text-sm">ทั้งหมด {exercises.length} ท่า</p>
            </div>
          </div>
          <button onClick={openAdd}
            ...
            <Plus className="w-5 h-5" />เพิ่มท่าใหม่
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 mb-4 flex flex-wrap gap-3">
          ...
          <select value={equipmentFilter} onChange={e => setEquipmentFilter(e.target.value)}
            ...
            {EQUIPMENT_LIST.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
          </select>
          ...
          <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
            ...
            {EXERCISE_CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
          </select>

          {/* ล้าง filter */}
          {(search || typeFilter !== "all" || equipmentFilter !== "all" || categoryFilter !== "all") && (
            <button
              onClick={() => { setSearch(""); setTypeFilter("all"); setEquipmentFilter("all"); setCategoryFilter("all"); }}
            ...
            <table className="w-full">
              <thead className="bg-slate-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left">การวัดผล</th>  {/* ✅ เพิ่ม */}
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan="6" className="text-center py-10 text-slate-400">กำลังโหลด...</td></tr>
                ) : filteredExercises.length === 0 ? (
                  <tr><td colSpan="6" className="text-center py-10 text-slate-400">ไม่พบท่าออกกำลังกาย</td></tr>
                  ...
                  <tr key={ex._id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <img
                          src={ex.image ? `${IMAGE_URL}${ex.image}` : "https://via.placeholder.com/80"}
                          alt={ex.exercise_name}
                          className="w-14 h-14 rounded-xl object-cover border shadow"
                        />
                        <span className="font-semibold text-slate-800">{ex.exercise_name}</span>
                      </div>
                    </td>

                    {/* TYPE */}
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs text-white bg-gradient-to-r ${
                        TYPE_COLORS[ex.exercise_type] || "from-slate-400 to-slate-500"
                      }`}>
                        {ex.exercise_type}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 bg-slate-200 rounded-full text-xs">
                        {EQUIPMENT_LABEL_MAP[ex.equipment_type] || ex.equipment_type}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        CATEGORY_COLORS[ex.exercise_category] || "bg-gray-100 text-gray-500"
                      }`}>
                        {EXERCISE_CATEGORIES.find(c => c.value === ex.exercise_category)?.label || "ใช้น้ำหนัก (kg)"}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="max-w-xs">
                        {ex.description
                          ? <p className="text-sm text-slate-600 line-clamp-2">{ex.description}</p>
                          : <span className="text-slate-400 text-sm">ไม่มีรายละเอียด</span>
                        }
                      </div>
                    </td>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                {/* ชื่อท่า */}
                <div>
                  <label className="block mb-2 font-semibold">ชื่อท่า <span className="text-red-500">*</span></label>
                  ...
                  {errors.exercise_name && <p className="text-red-500 text-sm mt-1">{errors.exercise_name}</p>}
                </div>

                {/* ประเภทท่า */}
                <div>
                  <label className="block mb-2 font-semibold">ประเภทท่า <span className="text-red-500">*</span></label>
                  ...
                  {errors.exercise_type && <p className="text-red-500 text-sm mt-1">{errors.exercise_type}</p>}
                </div>

                {/* อุปกรณ์ */}
                <div>
                  <label className="block mb-2 font-semibold">อุปกรณ์ <span className="text-red-500">*</span></label>
                  <select name="equipment_type" value={form.equipment_type} onChange={handleChange}
                    className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-emerald-400 outline-none">
                    <option value="">-- เลือกอุปกรณ์ --</option>
                    {EQUIPMENT_LIST.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}
                  </select>
                ...
                <div>
                  <label className="block mb-2 font-semibold">
                    ประเภทการวัดผล
                    <span className="text-xs text-gray-400 font-normal ml-2">(ใช้สำหรับหน้าบันทึกผลการฝึก)</span>
                  </label>
                  <select name="exercise_category" value={form.exercise_category} onChange={handleChange}
                    className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-emerald-400 outline-none">
                    {EXERCISE_CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                  </select>
                ...
                {/* รูปภาพ */}
                <div>
                  <label className="block mb-2 font-semibold">รูปท่าออกกำลังกาย</label>
                ...
                {form.image && (
                  <img
                    src={form.image instanceof File ? URL.createObjectURL(form.image) : `${IMAGE_URL}${form.image}`}
                    alt="preview"
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button type="submit"
                    ...
                    <CheckCircle className="w-5 h-5" />บันทึก
                  </button>
                </div>
              </form>
            </div>
